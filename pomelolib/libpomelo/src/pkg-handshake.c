...
static int pc__handshake_ack(pc_client_t *client, const char *data, size_t len);
...
int pc__handshake_req(pc_client_t *client) {
  ...
  if(handshake_opts) {
    ...
    json_t *handshake_opts_sys = json_object_get(handshake_opts, "sys");
    if(handshake_opts_sys) {
      json_t *clientKey = json_object_get(handshake_opts_sys, "clientKey");
      if(clientKey) {
          client->dh = createdh();
          int blen;
          unsigned char * ck = getpublickey(client->dh, &blen);
          char * out_key;
          int len = base64Encode(ck, blen, &out_key);
          json_object_set(sys, "clientKey", json_string(out_key));
          free(out_key);
      }
    }
  }
...
int pc__handshake_resp(pc_client_t *client,
  ...
  if(sys) {
      ...
      json_t * challenge_docs = json_object_get(sys, "challenge");
      if (serverkey) {
          unsigned char *out;
          const unsigned char * sk = (const unsigned char *)(json_string_value(serverkey));
          int len = base64Decode(sk, strlen(sk), &out);
  ...
  if(!status) {
      if(encrypt_challenge){
          char ack[1024] = {0};
          sprintf(ack, "{\"challenge\":\"%s\"}", encrypt_challenge);
          pc__handshake_ack(client, ack, strlen(ack));
          free(encrypt_challenge);
          encrypt_challenge = NULL;
      }
      else{
          pc__handshake_ack(client, NULL, 0);
      }
  }
...
static int pc__handshake_ack(pc_client_t *client, const char *data, size_t len) {
  pc_buf_t buf = pc_pkg_encode(PC_PKG_HANDSHAKE_ACK, data, len);
  if(buf.len == -1) {
